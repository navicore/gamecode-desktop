...
impl AgentManager {
    ...
    pub async fn process_input(&mut self, input: &str) -> Result<AgentResponse, String> {
        ...
        // Process with LLM
        let backend_response = self.backend.generate_response(&context).await
        ...
        self.context_manager.add_assistant_message(&backend_response.content);
    ...
    async fn execute_tool_calls(&self, tool_calls: Vec<ToolCall>) -> Result<Vec<ToolResult>, String> {
        let mut results = Vec::new();
        
        for tool_call in tool_calls {
            let result = self.tool_registry.execute_tool(&tool_call.name, &tool_call.args).await
    ...
    async fn maybe_compress_context(&mut self) -> Result<(), String> {
        if self.context_manager.context_length() > self.config.max_context_length {
            ...
            let summary_response = self.backend.generate_response(&summarization_prompt).await
                .map_err(|e| format!("Context compression error: {}", e))?;
                
            // Replace older context with summary
            self.context_manager.replace_with_summary(&summary_response.content);
        }
