...
use bevy_egui::egui::{Align, Frame, Layout, Rect, Stroke, Color32};

use crate::ui;
use crate::visualization::{self, VisualizationPlugin, VisualizationState, ToolStatus};
use crate::core;
use crate::agent;
    ...
    fn default() -> Self {
        Self {
            ...
            journal_messages: vec![
                ...
                JournalMessage {
                    content: "Type in the input box below to interact with the AI agent.".to_string(),
                    sender: MessageSender::System,
                    timestamp: 0.0,
                },
            ],
...
fn setup_system(mut commands: Commands, windows: Query<&Window>) {
    ...
    println!("Camera positioned at Y={} with viewport at {},{} size {}x{}", 
...
fn demo_tool_system(
    ...
    if time.elapsed_seconds() - app_state.last_demo_tool_time > 5.0 {
        ...
        visualization::start_tool_visualization(
            &mut commands,
            &mut vis_state,
            &tool_id,
            tool_type,
        ...
        app_state.journal_messages.push(JournalMessage {
            content: format!("<!-- TOOL_TRACKER:{}:{}:{} -->", tool_id, tool_type, current_time),
            sender: MessageSender::System,
            timestamp: current_time,
        });
    }
                    ...
                    if current_time - start_time > 3.0 {
                        ...
                        let already_completed = app_state.journal_messages.iter().any(|m| {
                            if let MessageSender::Tool(_) = m.sender {
                                m.content.contains(&format!("Completed {} tool (ID: {})", tool_type, tool_id)) ||
                                m.content.contains(&format!("Failed {} tool (ID: {})", tool_type, tool_id))
                            } else {
                                false
                            }
                        });
    ...
    for (tool_id, tool_type) in tools_to_complete {
        ...
        visualization::update_tool_status_public(
            ...
            &mut vis_state,
            &tool_id,
            if success { ToolStatus::Completed } else { ToolStatus::Failed },
            &mut tool_query,
        );
...
fn ui_system(
    ...
    egui::Window::new("Journal")
        ...
        .fixed_rect(egui::Rect::from_min_max(
            ...
            egui::pos2(available_rect.width(), visualization_height + journal_height),
        ))
        .show(ctx, |ui| {
                    ...
                    egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            ...
                            let (text_color, prefix) = match &message.sender {
                                ...
                                MessageSender::Assistant => (egui::Color32::from_rgb(100, 200, 255), "🤖 "),
                                MessageSender::System => (egui::Color32::GRAY, "📋 "),
                                MessageSender::Tool(tool_type) => {
                                    let color = match tool_type.as_str() {
                                        "file" => egui::Color32::from_rgb(100, 255, 100),
                                        "network" => egui::Color32::from_rgb(100, 200, 255),
                                        "process" => egui::Color32::from_rgb(255, 255, 100),
                                        "database" => egui::Color32::from_rgb(255, 100, 100),
                            ...
                            ui.horizontal(|ui| {
                                let formatted_text = format!("{}{}", prefix, message.content);
    ...
    egui::Window::new("Input")
        ...
        .fixed_rect(egui::Rect::from_min_max(
            ...
            egui::pos2(available_rect.width(), visualization_height + journal_height + input_height),
        ))
        .show(ctx, |ui| {
            ...
            ui.with_layout(Layout::top_down(Align::LEFT), |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    let text_edit = egui::TextEdit::multiline(&mut app_state.input_text)
                        ...
                        .hint_text("Type your command here... (Enter to submit, Shift+Enter for new line)")
                        .frame(true)
                    ...
                    if response.has_focus() && ui.input(|i| {
                        ...
                        !i.modifiers.shift // Not holding shift
                    }) {
